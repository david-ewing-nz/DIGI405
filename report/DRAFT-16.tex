...
{\small Summer Math Project --- Graduate Optimisation Course Supplement}}
\author{David Ewing}
\date{\today}
...
\subsection{Connections to Optimisation Theory}
\label{sec:opt_connections}

A reader coming from a graduate optimisation course will recognise the structure of variational inference immediately once it is cast in optimisation terms. This subsection draws that connection explicitly, before the general VI machinery is introduced in subsequent subsections.

\paragraph{From point estimation to distributional inference.}
A natural starting point is \emph{maximum a posteriori} (MAP) estimation, a standard optimisation problem. Given observed data $x$ and parameters $z$, MAP finds the single parameter value that maximises the posterior:
\[
  z^* = \arg\max_{z} \, p(z \mid x)
       = \arg\max_{z} \bigl[ \log p(x \mid z) + \log p(z) \bigr].
\]
The second form is recognisable as a penalised objective: maximise fit to data (log-likelihood) whilst regularising towards the prior (log-prior). This is exactly the structure of ridge regression, the lasso, or any regularised maximum likelihood problem. Numerical analysts solve it with gradient ascent, Newton's method, or quasi-Newton methods; the constraint set is all of $\mathbb{R}^d$.

MAP collapses the entire posterior to a single point, discarding all information about uncertainty. Full Bayesian inference instead seeks the complete posterior distribution $p(z \mid x)$, which represents how probability is spread across all plausible parameter values given the data. For uncertainty quantification, credible intervals, or downstream decision-making, the full posterior is needed. The challenge is that computing $p(z \mid x) \propto p(x \mid z)\,p(z)$ requires evaluating the normalising constant $p(x) = \int p(x \mid z)\,p(z)\,\mathrm{d}z$, an integral that is intractable in all but the simplest models.

\paragraph{Variational inference as constrained distributional optimisation.}
Variational inference converts intractable posterior computation into a tractable optimisation problem. We choose a family of distributions $\mathcal{Q}$---the \emph{variational family} or \emph{feasible set}---and find the member of that family closest to the true posterior:
\begin{equation}
  \min_{q \in \mathcal{Q}} \; \mathrm{KL}\bigl(q(z) \,\|\, p(z \mid x)\bigr),
  \label{eq:vi_opt}
\end{equation}
where the Kullback--Leibler divergence $\mathrm{KL}(q \| p) = \mathbb{E}_q[\log q(z) - \log p(z \mid x)]$ is a non-negative measure of discrepancy between distributions, equal to zero if and only if $q = p$ almost everywhere. Problem~\eqref{eq:vi_opt} is a constrained optimisation problem over a space of distributions: $\mathcal{Q}$ is the feasible region, and we seek the feasible point closest to the (infeasible) true posterior. When $\mathcal{Q}$ is well-chosen, this problem is far more tractable than exact inference.

\paragraph{The evidence lower bound as a surrogate objective.}
Problem~\eqref{eq:vi_opt} is still not directly computable because $\log p(z \mid x) = \log p(x, z) - \log p(x)$ involves the intractable $\log p(x)$. Standard optimisation practice replaces an intractable objective with a tractable surrogate. The relevant identity is
\[
  \log p(x) = \underbrace{\mathbb{E}_q[\log p(x,z)] - \mathbb{E}_q[\log q(z)]}_{\displaystyle\mathcal{L}(q) \;=\; \text{ELBO}(q)}
             + \mathrm{KL}\bigl(q(z) \,\|\, p(z \mid x)\bigr).
\]
Since $\mathrm{KL} \geq 0$, we have $\log p(x) \geq \mathcal{L}(q)$ always: the \emph{evidence lower bound} (ELBO) $\mathcal{L}(q)$ is a lower bound on the log model evidence. Crucially, because $\log p(x)$ is a constant with respect to $q$, minimising $\mathrm{KL}(q \| p)$ over $q \in \mathcal{Q}$ is exactly equivalent to maximising $\mathcal{L}(q)$ over $q \in \mathcal{Q}$. The ELBO is the tractable surrogate:
\[
  \mathcal{L}(q)
  = \underbrace{\mathbb{E}_q[\log p(x,z)]}_{\text{expected log-joint (fit)}}
  + \underbrace{H(q)}_{\text{entropy (spread)}},
\]
where $H(q) = -\mathbb{E}_q[\log q(z)]$ is the differential entropy of $q$. The ELBO has the same two-term structure as a MAP objective: the first term rewards placing mass on configurations of $z$ that explain the data; the second penalises overly peaked, overconfident distributions. Optimising the ELBO thus balances fit against uncertainty---the distributional analogue of regularised optimisation.

\paragraph{Coordinate ascent as the algorithm.}
The dominant algorithm for ELBO maximisation under mean-field factorisation $q(z) = \prod_j q_j(z_j)$ is \emph{coordinate ascent variational inference} (CAVI). This is precisely block coordinate ascent in the variational parameter space: cycle through the factors $q_1, q_2, \ldots$, updating each in turn whilst holding the others fixed. For conjugate exponential family models, each coordinate update has a closed-form solution. For non-conjugate models, the coordinate update is replaced by a gradient step (stochastic gradient ascent on the ELBO).

The convergence guarantee is the same as for any non-convex coordinate descent: the algorithm is guaranteed to reach a \emph{local} maximum of the ELBO, not necessarily the global one. Multiple initialisations and ELBO monitoring are standard practice---the same cautions an optimisation student would apply to any non-convex problem. Table~\ref{tab:opt_analogy} summarises the correspondence between standard optimisation concepts and their variational inference counterparts.

\begin{table}[htbp]
\centering
\caption{Correspondence between optimisation and variational inference concepts.}
\label{tab:opt_analogy}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ll}
\hline
\textbf{Optimisation concept} & \textbf{Variational inference counterpart} \\
\hline
Decision variable $x \in \mathbb{R}^d$ & Variational distribution $q \in \mathcal{Q}$ \\
Feasible set (constraints) & Variational family $\mathcal{Q}$ \\
Objective function $f(x)$ & ELBO $\mathcal{L}(q)$ (maximise) or KL $\mathrm{KL}(q\|p)$ (minimise) \\
Regularisation / penalty & Entropy term $H(q)$ \\
Surrogate / lower bound & ELBO as lower bound on $\log p(x)$ \\
Block coordinate descent & Coordinate ascent variational inference (CAVI) \\
Local minimum & Local maximum of ELBO \\
MAP estimation & Point-mass $q = \delta_{z^*}$ --- the degenerate special case \\
\hline
\end{tabular}
\end{table}

With this optimisation framing in hand, the remainder of the Methods section gives the mathematical details of VI, the variational families used in this paper, and the models to which they are applied. Throughout, the paper benchmarks VI against Gibbs sampling (a Markov Chain Monte Carlo method that draws exact posterior samples by cycling through the full conditional distribution of each parameter in turn; see Appendix A for details). This provides the gold standard against which VI's accuracy and speed can be assessed.
