...
To illustrate Variational Bayes, we look at an agricultural dataset, which can be downloaded as \texttt{farmdata.csv} (for weaning weights) and \texttt{Kmat.csv} (for the kinship matrix) from Canvas. To benchmark Variational Bayes, we will compare results to those obtained using Gibbs sampling.
...
\paragraph{Writing E-M algorithm to find V-B estimates}

To determine the parameters of the Variational Bayes approximation, we will use an Expectation-Maximisation algorithm. This means we will succesively update $E(\bm \beta), \text{Var}(\bm \beta), E({\bf u}), \text{Var}({\bf u}), \tau_u, \tau_e$ until convergence has been reached. In this code, convergence is defined as when $\sqrt{(\theta^{(t)}-\theta^{(t-1)})^2}/(|\theta^{(t)}|+0.01) \leq \epsilon$ for all elements of $\bm \theta$. The addition of 0.01 is to avoid problems when $\theta^{(t)} = 0$
...
curve(dnorm(x,mean=test1$beta_mean[1],sd=sqrt(test1$beta_var[1,1])),ylab='Density',main='',xlim=mlim,col=2,lty=1,xlab=expression(beta[1]),cex.lab=2.5,cex.axis=1.5)
...
curve(dnorm(x,mean=test1$beta_mean[2],sd=sqrt(test1$beta_var[2,2])),ylab='Density',main='',xlim=mlim,col=2,lty=1,xlab=expression(beta[2]),cex.lab=2.5,cex.axis=1.5)
...
curve(dnorm(x,mean=test1$u_mean[i],sd=sqrt(test1$u_var[i,i])),ylab='Density',main='',xlim=mlim,col=2,lty=1,xlab=paste('u',i,sep=''),cex.lab=2.5,cex.axis=1.5)
