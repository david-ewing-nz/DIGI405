...
for (i in 1:p) {
  ...
  if (!is.null(gibbs_s1)) {
    ...
    # Create plot
    p_beta <- ggplot() +
      geom_density(data = data.frame(x = gibbs_beta), 
           aes(x = x, fill = "Gibbs", colour = "Gibbs"), 
           alpha = 0.4, linewidth = 1.2) +
      geom_line(data = df_vb, aes(x = x, y = density, color = "VB"), 
            linewidth = 1.2) +
      scale_fill_manual(values = c(Gibbs = "#E7298A")) +
      scale_colour_manual(values = c(VB = "black", Gibbs = "#E7298A")) +
...
if (!is.null(gibbs_s1)) {
  ...
  p_tau <- ggplot() +
    geom_density(data = data.frame(x = gibbs_tau_e), 
           aes(x = x, fill = "Gibbs", colour = "Gibbs"), 
           alpha = 0.4, linewidth = 1.2) +
    geom_line(data = df_vb_tau, aes(x = x, y = density, color = "VB"), 
          linewidth = 1.2) +
    scale_fill_manual(values = c(Gibbs = "#E7298A")) +
    scale_colour_manual(values = c(VB = "black", Gibbs = "#E7298A")) +
...
```{r load_dr_john_rds, eval=FALSE, fig.width=10, fig.height=8}
...
```{r save_sd_ratios_m1, echo=FALSE}
# Save SD ratios to shared RDS file for comparison
if (!is.null(gibbs_s1) && exists("comparison_df")) {
  sd_row <- data.frame(
    Model = "M1",
    Q = NA,
    beta_0 = comparison_df$SD_Ratio[comparison_df$Parameter == "beta0"],
    beta_1 = comparison_df$SD_Ratio[comparison_df$Parameter == "beta1"],
    beta_2 = comparison_df$SD_Ratio[comparison_df$Parameter == "beta2"],
    tau_e = comparison_df$SD_Ratio[comparison_df$Parameter == "tau_e"],
    tau_u = NA,
    sigma2_e = NA,
    sigma2_u = NA,
  ...
  if (file.exists(rds_path)) {
    ...
    all_sd <- all_sd[all_sd$Model != "M1", ]
    all_sd <- rbind(all_sd, sd_row)
  } else {
    all_sd <- sd_row
  }
  
  saveRDS(all_sd, rds_path)
  cat("\nSaved M1 SD ratios to", rds_path, "\n")
}
```
