...
ggsave("../figs/M2_temp_barplot.png", temp_plot, width = 6, height = 4, dpi = 300)
```
...
ggsave(
  filename = "../figs/M2_s1_convergence.png",
  plot     = conv_plot_s1,
  width    = plot_width,
  height   = 6,
  dpi      = 300
...
img_conv <- readPNG("../figs/M2_s1_convergence.png")
grid.newpage()
...
img_elbo <- readPNG("../figs/M2_s1_elbo.png")
grid.newpage()
...
plot_name_s1 <- "M2_vb_Q5_8Panel.png"
ggsave(
...
ggsave(
  filename = "../figs/M2_s2_convergence.png",
  plot     = conv_plot_s2,
  width    = 12,
  height   = 6,
  dpi      = 300
...
img_conv2 <- readPNG("../figs/M2_s2_convergence.png")
grid.newpage()
...
img_elbo2 <- readPNG("../figs/M2_s2_elbo.png")
grid.newpage()
...
ggsave(
  filename = "../figs/M2_vb_Q50_8Panel.png",
...
img_s2 <- readPNG("../figs/M2_vb_Q50_8Panel.png")
grid.newpage()
grid.raster(img_s2)
```

# scenario 3: Q=20 (M3 only)
...
knitr::include_graphics("../figs/M2_my_tau_u_comparison.png")
```
...
rds_plot_path <- "../figs/M2_my_from_rds_tau_u_comparison.png"
...
```{r save_sd_ratios_m2, echo=FALSE}
if (run_gibbs && length(results_multi) > 0) {
  ...
  if (nrow(sd_rows) > 0) {
    ...
    if (file.exists(rds_path)) {
      ...
      all_sd <- all_sd[!grepl("^M2_", all_sd$Model), ]
    ...
    cat("\nSaved M2 SD ratios for", nrow(sd_rows), "configurations\n")
  }
}
```

```{r save_timing_data_m2, echo=FALSE}
# Save timing data for M2 (all 5 scenarios when M3 enabled)
if (model_type == "M3") {
  timing_m2 <- data.frame(
    Model = c("M2_Q5", "M2_Q10", "M2_Q20", "M2_Q50", "M2_Q100"),
  ...
  timing_m2 <- data.frame(
    Model = c("M2_Q5", "M2_Q10"),
    Q = c(q_s1, q_s2),
    vb_time = c(vb_time_s1[3], vb_time_s2[3]),
    gibbs_time = if(run_gibbs) c(gibbs_time_s1[3], gibbs_time_s2[3]) else c(NA, NA),
    speedup = if(run_gibbs) c(gibbs_time_s1[3]/vb_time_s1[3], gibbs_time_s2[3]/vb_time_s2[3]) else c(NA, NA),
    stringsAsFactors = FALSE
  )
}
...
if (file.exists(timing_rds_path)) {
  ...
  all_timing <- all_timing[!grepl("^M2_", all_timing$Model), ]
  all_timing <- rbind(all_timing, timing_m2)
} else {
  all_timing <- timing_m2
}

saveRDS(all_timing, timing_rds_path)
cat("\nSaved M2 timing data for", nrow(timing_m2), "configurations\n")
if (model_type == "M3") {
  cat(sprintf("  Q=5:   VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n", 
      timing_m2$vb_time[1], timing_m2$gibbs_time[1], timing_m2$speedup[1]))
  cat(sprintf("  Q=10:  VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n",
      timing_m2$vb_time[2], timing_m2$gibbs_time[2], timing_m2$speedup[2]))
  cat(sprintf("  Q=20:  VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n",
      timing_m2$vb_time[3], timing_m2$gibbs_time[3], timing_m2$speedup[3]))
  cat(sprintf("  Q=50:  VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n",
      timing_m2$vb_time[4], timing_m2$gibbs_time[4], timing_m2$speedup[4]))
  cat(sprintf("  Q=100: VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n",
      timing_m2$vb_time[5], timing_m2$gibbs_time[5], timing_m2$speedup[5]))
} else {
  cat(sprintf("  Q=5:  VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n", 
      timing_m2$vb_time[1], timing_m2$gibbs_time[1], timing_m2$speedup[1]))
  cat(sprintf("  Q=10: VB=%.3fs, Gibbs=%.3fs, Speedup=%.1fx\n",
