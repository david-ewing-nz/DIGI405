...
plot_vb_posteriors <- function(mu_beta, Sigma_betau,
  ...
  # Skip tau_e if NULL (for logistic model)
  # ============================================================================
  tau_names <- c("tau_e", "tau_u")
  if (is.null(E_tau_e)) {
    tau_names <- c("tau_u")  # Logistic model: only tau_u
  }
  ...
  for (tau_name in tau_names) {
    ...
    # Skip if this tau parameter is NULL
    if (is.null(E_tau)) next
    
    if (run_gibbs && !is.null(gibbs_tau)) {
